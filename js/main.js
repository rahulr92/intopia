...
function validate_nec_frm(){
  var submit_flag = 1;
  //add other checks
  if(!$('input[name=e-signature-nec]').is(':checked')){
    alert("Oops, you forgot to sign your NEC Purchase agreement. Please try again.!");
    return false;
  }

  $('select').each(function(){

    if($(this).val()=="left blank")
      submit_flag = 0;

  });

 if(!$('input[name=unit-price]').val()){
    submit_flag = 0
  }


 if(!$('input[name=cash-payment]').val()){
    submit_flag = 0
  }

 if(!$('input[name=unit-quantity]').val()){
    submit_flag = 0
  }
  
  if(submit_flag == 0)
  {
    alert("Please complete all options marked with a red * to submit your NEC Purchase form!");
    return false;
